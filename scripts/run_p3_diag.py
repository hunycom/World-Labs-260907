#!/usr/bin/env python3
"""
scripts/run_p3_diag.py

Drives browser to http://localhost:8080/?p3_pick_eval=t5b_diag
to capture harness diagnosis state before and after fix.
"""
...
DIAG_JSON = RAW_DIR / 't5b_harness_diag.json'
DIAG_TXT = RAW_DIR / 't5b_harness_diag.txt'
...
def run_diag(label=""):
    print(f"[*] Navigating Firefox to run harness diagnosis ({label})...")
    if DIAG_JSON.exists():
        DIAG_JSON.unlink()
    if DIAG_TXT.exists():
        DIAG_TXT.unlink()

    url = f'http://localhost:8080/?p3_pick_eval=t5b_diag&t={int(time.time())}'
    navigate_browser(WINDOW_ID, url)

    for i in range(30):
        time.sleep(2)
        if DIAG_JSON.exists() and DIAG_JSON.stat().st_size > 50:
            print(f"\n[+] Diagnosis received ({label})!")
            break
        print(f"    [{(i+1)*2}s] Waiting...", end="\r", flush=True)
