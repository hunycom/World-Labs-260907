...
REPORT_PATH = os.path.join(BASE_DIR, "docs/phase2_results.json")
WINDOW_ID = "0x2800016"
DISPLAY_STR = ":11.0"
...
def navigate_single_tab_foreground(win_id_hex, url):
    ...
    print(f"[ORCHESTRATOR-P2] Successfully navigated single-tab Firefox window {win_id_hex} to {url}")

print(f"[ORCHESTRATOR-P2] Commencing Phase 2 Rigid Dynamics Benchmark on Window {WINDOW_ID}...")
audit_url = f"http://localhost:8080/?phase2_audit=true&t={int(time.time())}"
navigate_single_tab_foreground(WINDOW_ID, audit_url)

print("[ORCHESTRATOR-P2] Monitoring Phase 2 audit progress in foreground single-tab...")
t_start = time.time()
grounded_captured = False

while time.time() - t_start < 180:
    elapsed = time.time() - t_start
    if elapsed >= 20.0 and not grounded_captured:
        print(f"[ORCHESTRATOR-P2] Capturing Grounded Settled Screenshot at {elapsed:.1f}s...")
        subprocess.run([sys.executable, CAPTURE_SCRIPT, WINDOW_ID, os.path.join(BASE_DIR, "docs/screenshot_phase2_grounded_window.png"), DISPLAY_STR])
        grounded_captured = True

    if os.path.exists(REPORT_PATH):
        print(f"[ORCHESTRATOR-P2] Phase 2 report generated at {REPORT_PATH} in {elapsed:.1f}s!")
        break
    time.sleep(1.0)
...
if os.path.exists(REPORT_PATH):
    ...
    if data:
        ...
        print("[ORCHESTRATOR-P2] Warning: Could not parse report.")
else:
    print("[ORCHESTRATOR-P2] Warning: Timed out waiting for report.")
